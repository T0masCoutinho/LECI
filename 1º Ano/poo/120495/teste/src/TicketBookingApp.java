...
public class TicketBookingApp {
	...
	private static void exercise2(PrintStream out) throws IOException {
		out.println("\n\nAlínea 2) ----------------------------------\n");
		...
		*/
        ...
        File f = new File("src/ticket_bookings.txt");
        Scanner sc = new Scanner(f);

        while (sc.hasNextLine()) {
            String entryLine = sc.nextLine();
            if (!entryLine.startsWith("##")) {
                String[] entrySplitted = entryLine.split(",");
                int eventId = Integer.parseInt(entrySplitted[0].substring(8));
                int quantity = Integer.parseInt(entrySplitted[1].strip());

                tbs.bookTickets(eventId, quantity);
            }
        }
		...
		*/
        ...
        out.println(tbs.getFirst10AvailableEventsSortedByDate());
